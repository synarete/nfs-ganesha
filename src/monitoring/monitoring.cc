...
namespace ganesha_monitoring
{
  ...
  Metrics::Metrics(prometheus::Registry &registry) : // Counters
  ...
  extern "C"
  {
    ...
    void monitoring_init(const uint16_t port)
    {
      ...
      LogEvent(COMPONENT_MONITORING, "Init monitoring at %s", hostPort.c_str());
    ...
    void monitoring_worker_thread_min_increment(const uint32_t value)
    {
      metrics->workerThreads.Add({{"label", "min"}}).Increment(value);
    }

    void monitoring_worker_thread_min_decrement(const uint32_t value)
    {
      metrics->workerThreads.Add({{"label", "min"}}).Decrement(value);
    }

    void monitoring_worker_thread_max_increment(const uint32_t value)
    {
      metrics->workerThreads.Add({{"label", "max"}}).Increment(value);
    }

    void monitoring_worker_thread_max_decrement(const uint32_t value)
    {
      metrics->workerThreads.Add({{"label", "max"}}).Decrement(value);
    }

    void monitoring_worker_thread_start()
    {
      metrics->workerThreads.Add({{"label", "total"}}).Increment();
    }

    void monitoring_worker_thread_exit()
    {
      metrics->workerThreads.Add({{"label", "total"}}).Decrement();
    }
