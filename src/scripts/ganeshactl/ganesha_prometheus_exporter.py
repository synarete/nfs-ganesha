...
# -*- coding: utf-8 -*-
"""
Prometheus metrics exporter for NFS-ganesha.

Admin's utility program to export NFS-ganesha stats as Prometheus metrics.
Uses the same Ganesha.glib_dbus_stats API as ganesh_stats, but instead of
printing in human-readable format, exports the imporation as Prometheus
metrics gauges.

By default, uses port 8080 to export metrics. On a containerized environment
(e.g., Kubernetes) may run as a sidecar container and share the DBus socket
with NFS-ganesha main container.

Run with:
  $ python ganesha_prometheus_exporter.py
...
class ExportCollector(Collector):
    ...
    def collect_export_stats(self):
        export_stats = self.dbus_if.export_stats()
        gauge = self.make_gauge(
            "count",
            "Number of active exports",
        )
        gauge.add_metric([], len(export_stats.exports))
        yield gauge
        for exportid, export in export_stats.exports.items():
            exid = ganesha_dbus.dbus_to_std(exportid)
            path = ganesha_dbus.dbus_to_std(export.path)
            yield from self.collect_total_stats(exid, path)

    def collect_total_stats(self, export_id, export_path):
        total_stats = self.dbus_if.total_stats(export_id)
        for key in total_stats.stats:
            if not total_stats.stats[key][0]:
                return
            if total_stats.stats[key][1] != "OK":
                continue
            for i in range(0, len(total_stats.stats[key][3]) - 1, 2):
                proto = str(total_stats.stats[key][3][i])
                count = int(total_stats.stats[key][3][i + 1])
                gauge = self.make_gauge(
                    "total_stats_" + proto.lower(),
                    "Total stats for " + proto,
                    labels=["exportid", "path"],
                )
                gauge.add_metric([str(key), str(export_path)], count)
                yield gauge
